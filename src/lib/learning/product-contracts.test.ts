...
import { DAILY_PLAN_SLOTS, TRAINING_MODULES } from '../../components/trainingCatalog';
...
describe('product learning contracts', () => {
  it('provides multiple repeatable exercises for every catalog module', () => {
    for (const module of TRAINING_MODULES) expect(DAILY_PLAN_SLOTS.filter((exercise) => exercise.module === module.module).length).toBeGreaterThan(1);
  });
  ...
  it('keeps Today finite while the generator can continue selecting module work', () => {
    ...
    const plan = generateDailyPlan({ userId: 'test-user', candidates: DAILY_PLAN_SLOTS, progress, attempts: [attempt('board-grip', 'board-grip-daily-1')] });
    expect(plan.items.length).toBeGreaterThan(0);
    const generator = new PoolExerciseGenerator({ list: (module) => validatedExercises(module) });
    expect(generator.generate({ module: 'calculation', excludeFingerprints: new Set() })).not.toBeNull();
  });
});
